...
const CounterColis = ({userId, userToken}) => {
    ...
    useEffect(() => {
      ...
      const fetchColis = async () => {
          try {
              const response = await axios.get(config.baseUrl + '/get_colis/' + userId, {
                  headers: {
              ...
              setTraitement(response.data.filter((item) => item.status_colis === 'Traitement').length);
          } catch (error) {
...
icon: (
  <img
    style={imgStyle}
    src="https://cdn-icons-png.freepik.com/512/6462/6462897.png"
    alt="icon"
  />
),
}}
...
export default connect(mapStateToProps) (CounterColis);
